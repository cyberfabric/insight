...
**Tenant-specific custom metrics** — v1 assumes all metrics are product-defined and global. A metric that exists only for one tenant (say, a bank's `mas_compliance_score` or a healthcare tenant's HIPAA-driven KPI) is not supported in v1. This is separate from "tenant wants a different threshold on an existing metric" (which v1 does support) — it's "tenant wants a whole new metric nobody else has", which requires tenant-specific `query_ref` in `analytics.metrics`, tenant-aware catalog rows, and possibly tenant-specific silver/gold transforms. Cross-layer problem, deserves its own PRD. The v1 global `metric_catalog` shape does not preclude either extension path (nullable `tenant_id` column, or a parallel `metric_catalog_tenant_custom` table) — both are additive.

---

## Architectural decisions that matter

**Global `metric_catalog`, per-tenant `metric_threshold`.** Metadata is identical across tenants by policy — we forbid per-tenant metadata edits in v1 — so keying metadata per tenant would duplicate every row N times for a feature we've ruled out. Thresholds are local policy and genuinely per-tenant.
