...
| Metric catalog | The MariaDB table that persists one row per `metric_key` (global, not per-tenant) with semantic metadata. Per-tenant thresholds live in a separate `metric_threshold` table. |
| Metric threshold | A row in `metric_threshold` that pins the `good`, `warn`, `alert_trigger`, or `alert_bad` boundary for a metric at some scope — company-wide (`tenant`), per-role (`role`), per-team (`team`), per-team-per-role (`team+role`), or the product-seeded floor (`product-default`). |
...
- `metric_catalog` table: one row **per `metric_key`** (global, not per-tenant) with fields `label_i18n_key`, `sublabel_i18n_key`, `description_i18n_key`, `unit`, `format`, `higher_is_better`, `is_member_scale`, `source_tags` (array of strings), `is_enabled`, timestamps. Per-tenant metadata overrides are out of v1 scope; if ever needed, an additive `metric_catalog_tenant_override` table lands without schema change to `metric_catalog`.
...
- **Tenant-specific custom metrics** — a metric that exists for one tenant only (e.g., a bank's regulator-mandated `mas_compliance_score`, a healthcare tenant's HIPAA-driven KPI). v1 assumes all metrics are product-defined and global; the mechanism to add per-tenant-only metrics does not exist. This is a cross-layer concern (tenant-specific `analytics.metrics.query_ref`, tenant-aware catalog rows, possibly tenant-specific silver/gold transforms) that warrants its own PRD. The v1 global `metric_catalog` shape does not preclude either extension path — see §13 Open Questions for the two candidate approaches
...
The system **MUST** persist one row **per `metric_key`** in `metric_catalog` with at least: `label_i18n_key`, `sublabel_i18n_key`, `description_i18n_key`, `unit`, `format`, `higher_is_better`, `is_member_scale`, `source_tags` (array of strings), `is_enabled`, `created_at`, `updated_at`. `metric_key` **MUST** be the primary key (no `tenant_id` column on `metric_catalog`). In v1 rows are created by seed migrations; the table shape is intentionally friendly to future population from an external source (e.g., silver-plugin manifests registering their metrics at install time) without requiring a schema change.

**Rationale**: Metric metadata (labels, units, format, `higher_is_better`) is product-level contract, identical across tenants by design (see `cpt-metric-cat-fr-metadata-writes`). Keying `metric_catalog` globally avoids duplicating every row across every tenant — 200 metrics × 1000 tenants = 200 000 identical rows with identical content, paid for a capability v1 explicitly forbids. If per-tenant metadata overrides ever become a real use case (tenant admin rewriting a label, per-tenant localization), they land through an additive `metric_catalog_tenant_override` table without modifying `metric_catalog`. Per-tenant **thresholds** remain per-tenant because thresholds are local policy, not product contract — they live in `metric_threshold`. Seeding-via-migration in v1 does not lock the catalog into being an admin-edited free-form store; populating rows from declarative plugin manifests later is an additive change on the write side only.
...
- [ ] `metric_catalog` is keyed globally by `metric_key` (not `(tenant_id, metric_key)`); there is no `tenant_id` column on `metric_catalog` in v1
...
| **Tenant-specific custom metrics** — v1 is global-only. When the first enterprise customer asks for a metric unique to them (regulatory score, custom KPI), how do we extend? Two candidate approaches, both additive to the v1 schema: **(a)** add a nullable `tenant_id` column to `metric_catalog` — `tenant_id IS NULL` means global (current behavior unchanged), non-null means tenant-custom. Read endpoint filters by `tenant_id IN (caller_tenant, NULL)`. Simpler to implement, compound-uniqueness rules need thought. **(b)** separate `metric_catalog_tenant_custom` table with tenant-specific columns. Read endpoint UNIONs or merges. Cleaner separation but adds a join / union on every read. Deeper question: tenant-specific `query_ref` rows in `analytics.metrics` also need a story, and so might silver/gold transforms if the custom metric pulls from a custom source. This is a cross-layer problem, not just a catalog one. | Insight Product Team + Backend tech lead | When first customer asks (or earlier if Enterprise sales commits to it) |
