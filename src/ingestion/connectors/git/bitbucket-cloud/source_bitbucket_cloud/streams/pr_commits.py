"""Bitbucket Cloud PR commits stream (REST, per-PR fanout, concurrent, incremental)."""
...
class PRCommitsStream(BitbucketCloudRestStream):
    """Fetches commits linked to each PR via REST with pagination.

    API: GET /repositories/{workspace}/{repo_slug}/pullrequests/{id}/commits

    Per-PR incremental: only fetches commits for PRs whose updated_on is newer
    ...
    def _fetch_pr_commits(self, stream_slice: dict) -> List[Mapping[str, Any]]:
        """Fetch all commits for one PR with pagination. Thread-safe."""
        ...
        url: Optional[str] = f"{API_BASE}/repositories/{workspace}/{repo_slug}/pullrequests/{pr_id}/commits"
        ...
        while url:
            ...
            if not check_rest_response(resp, f"{workspace}/{repo_slug} PR#{pr_id} commits"):
                break
